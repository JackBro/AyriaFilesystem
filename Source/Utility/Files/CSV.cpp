...
std::vector<std::vector<std::string>> COAL::CSV::Readfile(std::string Filepath)
{
...
bool COAL::CSV::Writefile(std::vector<std::vector<std::string>> Buffer, std::string Filepath)
{
    ...
    for (size_t i = 0; i < Buffer[0].size(); ++i)
        fputs(COAL::va(" %c,", 0x41 + i), Filehandle);
    fputs("\n\n", Filehandle);
...
bool COAL::CSV::Manager::Readfile(std::string Filepath)
{
    Buffer = CSV::Readfile(Filepath);
    return Buffer.size() > 0;
}
bool COAL::CSV::Manager::Writefile(std::string Filepath)
{
    return CSV::Writefile(Buffer, Filepath);
}

std::string COAL::CSV::Manager::Value(size_t Row, size_t Col)
{
    // Check rowcount.
...
void COAL::CSV::Manager::Insert(std::vector<std::string> Line)
{
